...
<template>
  <view>
    <app-open
      v-for="(t,index) in data.openList"
      :key="index"
      :data='t'
      :index='index'
    >
      ...
      <view v-if="t">
        ...
        <view v-else-if="t.slot=='intellectSearch'">
          ...
          <view class="btn-wrapper">
            <view
              class="btn1"
              @click="slot(index,'goDetail')"
            >查看商品</view>
            <view
              class="btn2"
              @click="slot(index,'discount')"
            >立即购买</view>
          </view>
          <view
            class="close-box"
        ...
        <view v-else-if="t.slot=='pddAuthorize'">
          ...
          <view class="logo-wrapper">
            ...
            <image
              class="logo"
              :src="authorize.PDDLOGO"
            />
          </view>
          <view class="desc">拼多多账号授权后，自己下单和分享商品都能获得佣金哦！</view>
          <view
            class="authorize-btn"
            @click="slot(index,'goAuthorize')"
          >前往拼多多授权</view>
        </view>
        ...
        <!--外卖分享 -->
        <view v-else-if="t.slot=='shareTakeaway'">
          <view class="img-box">
            <image
              :src="t.imgUrl"
              mode="aspectFill"
            ...
            />
            <view class="desc">好友通过海报领券购买，您可赚取佣金</view>
          </view>
          <view class="btn-box">
            <view
              class="btn1"
            ...
            <view
              class="btn1"
              @click="slot(index,'preserve')"
            >保存海报</view>
          </view>
        </view>

        <!-- 用户提现 -->
        <view v-else-if="t.slot=='cashOut'">
          <image
            class="logo"
            src="static/logo-user.png"
          />
          <view class="title">提现需前往APP提现</view>
          <view class="title">可通过以下方式下载APP：</view>
          <view class="desc mt-10">1、通过应用市场或App Store搜索「呆萌价」下载。</view>
          <view class="desc">2、复制链接打开浏览器下载。</view>
          <button
            class="download-btn"
            @click="slot(index,'copy')"
          >复制链接</button>
          <view
            class="close-box"
            @click="app.openEsc()"
